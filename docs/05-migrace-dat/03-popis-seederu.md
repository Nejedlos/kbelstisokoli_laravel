...
1. `LegacyUserMigrationSeeder` – základní uživatelská data.
2. `SeasonMigrationSeeder` – identifikace a sjednocení sezón.
3. `EventMigrationSeeder` – zápasy, tréninky a klubové akce.
4. `AttendanceMigrationSeeder` – docházka a RSVP.
5. `FinanceMigrationSeeder` – tarify, konfigurace, platby a pokuty.
...
- **Klíčová logika:**
    - Mapování polí ze staré DB (např. `jmeno`, `prijmeni`, `email`).
...
- **Zdrojové tabulky:** `web_vypocty_platby`, `web_platici`, `web_platby`, `web_pokuty`
- **Cílové tabulky:** `financial_tariffs`, `user_season_configs`, `finance_charges`, `finance_payments`, `charge_payment_allocations`
- **Klíčová logika:**
    ...
    - **Předpisy (Charges):**
        ...
        - Migrace pokut jako samostatných předpisů typu `fine`.
