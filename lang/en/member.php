<?php

return [
    'dashboard' => [
        'profile' => [
            'payment_vs' => 'Variable Symbol',
            'member_id' => 'Member ID',
            'teams' => 'Teams',
            'no_teams' => 'Not assigned to any team yet',
        ],
        'actions' => [
            'edit_profile' => 'Edit profile',
            'my_program' => 'My program',
            'payments' => 'Payments',
        ],
        'activity' => [
            'title' => 'My Activity',
            'view_all' => 'View all',
            'empty' => 'No recent activity',
        ],
        'economy' => [
            'title' => 'Economy summary',
            'cta' => 'Open payments overview',
        ],
    ],
    'attendance' => [
        'title' => 'My Program',
        'my_status' => 'My attendance',
        'subtitle' => 'Overview of upcoming trainings, matches, and club events.',
        'no_events' => 'No upcoming events',
        'no_events_text' => 'You have no trainings or matches scheduled in the near future.',
        'no_events_past' => 'No past events found.',
        'no_events_filter' => 'No events found for the selected filters.',
        'view_history' => 'View my attendance history',
        'history_title' => 'Attendance History',
        'history_subtitle' => 'Overview of all your past responses and participations.',
        'no_history' => 'You have no attendance history yet.',
        'deadline_reached' => 'Attendance deadline has passed (90 mins before event).',
        'filter' => [
            'title' => 'Program Filter',
            'type' => 'Event Type',
            'year' => 'Year',
            'month' => 'Month',
            'attendance' => 'My Attendance',
            'period' => 'Period',
            'upcoming' => 'Upcoming',
            'past' => 'Past',
            'all' => 'All',
            'confirmed' => 'Attending',
            'declined' => 'Excused',
            'none' => 'Not specified',
            'maybe' => 'Maybe',
            'types' => [
                'all' => 'All Events',
                'training' => 'Trainings',
                'match' => 'Matches',
                'event' => 'Club Events',
            ],
            'months' => [
                '1' => 'January',
                '2' => 'February',
                '3' => 'March',
                '4' => 'April',
                '5' => 'May',
                '6' => 'June',
                '7' => 'July',
                '8' => 'August',
                '9' => 'September',
                '10' => 'October',
                '11' => 'November',
                '12' => 'December',
            ],
            'submit' => 'Filter',
            'reset' => 'Reset Filter',
        ],
        'table' => [
            'event' => 'Event',
            'date' => 'Date',
            'status' => 'Status',
            'note' => 'Note',
            'responded' => 'Responded',
        ],
        'status' => [
            'pending' => 'Not specified',
            'confirmed' => 'Attending',
            'declined' => 'Excused',
            'maybe' => 'Maybe',
        ],
        'event_types' => [
            'match' => 'Basketball match',
            'training' => 'Training',
            'event' => 'Event',
            'unknown' => 'Unknown event',
        ],
        'detail_title' => 'Event Detail',
        'back_to_program' => 'Back to Program',
        'notes_label' => 'Event Notes',
        'update_status_title' => 'Update My Attendance',
        'note_field' => 'Additional Note',
        'note_placeholder' => 'Optional...',
        'excuse_reason' => 'Reason for Absence',
        'excuse_reasons' => [
            'illness' => 'Illness',
            'injury' => 'Injury',
            'work' => 'Work / School',
            'family' => 'Family reasons',
            'holiday' => 'Holiday',
            'other' => 'Other',
        ],
        'save_response' => 'Save Response',
        'save_success' => 'Your response has been successfully saved.',
        'bulk_save_success' => 'Successfully bulk updated :count events.',
        'cannot_change_past' => 'Attendance for past events cannot be changed.',
        'bulk_actions' => [
            'title' => 'Bulk Actions',
            'confirm' => 'Mark as: Attending',
            'decline' => 'Excuse...',
            'decline_no_reason' => 'Decline without reason',
            'selected' => ':count events selected',
            'clear' => 'Clear Selection',
        ],
        'who_comes_title' => 'Who is Coming?',
        'who_not_comes_title' => 'Excused',
        'who_pending_title' => 'Not responded yet',
    ],
    'profile' => [
        'title' => 'My Profile',
        'subtitle' => 'Here you can edit your contact details and account settings.',
        'avatar' => [
            'title' => 'Avatar',
            'change' => 'Change',
            'hint' => 'Upload your own photo or choose one from our club gallery.',
            'formats' => 'We support JPG, PNG and WebP formats. Maximum size 5 MB.',
            'gallery_open' => 'Open gallery',
            'modal_title' => 'Avatar Management',
            'modal_subtitle' => 'Edit your current avatar or choose a brand new look.',
            'no_image_selected' => 'No image selected yet',
            'no_image_hint' => 'Choose a photo from the gallery or upload your own below.',
            'cancel_selection' => 'Cancel selection',
            'reset_crop' => 'Reset crop',
            'save_and_set' => 'Save and set',
            'tab_gallery' => 'Club Gallery',
            'tab_upload' => 'Upload Own',
            'gallery_empty' => 'Gallery is currently empty',
            'upload_click' => 'Click to choose photo',
            'timeout_title' => 'Time-out! Substituting?',
            'timeout_text' => 'Do you really want to send your current avatar <span class="text-primary font-black uppercase tracking-wider">to the bench</span>? After deletion, you will only see initials next to your name.',
            'keep_in_game' => 'Keep in game',
            'send_to_bench' => 'Send to bench',
            'info_footer' => 'Avatar is your face in the system. Coaches will see it on the roster and teammates in the attendance overview.',
            'admin' => [
                'delete_title' => 'Delete system avatar?',
                'delete_text' => 'This avatar will be permanently removed from the club gallery for all users.',
                'delete_confirm' => 'Yes, delete from gallery',
                'upload_as_system' => 'Upload as club avatar',
                'upload_as_system_hint' => 'Avatar will be added to the shared gallery for all members.',
                'flash' => [
                    'system_saved' => 'Club avatar has been successfully added to the gallery.',
                    'system_saved_bulk' => ':count club avatars have been successfully added to the gallery.',
                    'system_deleted' => 'Club avatar has been removed from the gallery.',
                ],
                'bulk_import_btn' => 'Import all to gallery',
            ],
            'bulk_ready_title' => 'Bulk import ready',
            'bulk_ready_subtitle' => 'You have selected multiple files for upload to the club gallery. Images will be automatically squared and optimized.',
            'flash' => [
                'saved' => 'Avatar has been successfully saved.',
                'deleted' => 'Avatar has been successfully sent to the bench.',
            ],
        ],
        'basic_info' => 'Basic Information',
        'name' => 'Full Name',
        'email' => 'Email Address',
        'email_help' => 'Email cannot be changed in the profile. Please contact the administrator.',
        'phone' => 'Phone Number',
        'jersey_number' => 'Jersey Number',
        'bio' => 'Public Bio / About Me',
        'bio_help' => 'This text may be displayed on the website on your profile.',
        'password_change' => 'Change Password',
        'password_help' => 'If you do not wish to change your password, leave these fields blank.',
        'current_password' => 'Current Password',
        'new_password' => 'New Password',
        'confirm_new_password' => 'Confirm New Password',
        'two_factor' => [
            'title' => 'Two-Factor Authentication (2FA)',
            'active' => 'Active',
            'inactive' => 'Inactive',
            'help' => 'Two-factor authentication adds an extra layer of security to your account. You will need to enter a verification code from a mobile app when logging in.',
            'admin_warning' => 'Important: As an administrator, you must have 2FA enabled to access the club management.',
            'enable' => 'Enable 2FA',
            'disable' => 'Disable 2FA',
            'regenerate_codes' => 'Regenerate Recovery Codes',
            'recovery_codes_title' => 'Your Recovery Codes',
            'recovery_codes_help' => 'Save these codes in a safe place. They will help you log in if you lose access to the app.',
            'setup_title' => 'Complete Setup',
            'setup_step_1' => 'Install an app (e.g., Google Authenticator).',
            'setup_step_2' => 'Scan this QR code in your app.',
            'setup_step_3' => 'Enter the 6-digit code displayed in the app.',
            'manual_key' => 'Manual Key',
            'code_label' => 'Verification Code',
            'confirm_button' => 'Confirm',
        ],
        'save_changes' => 'Save Changes',
        'update_success' => 'Your profile has been successfully updated.',
        'avatar_updated' => 'Avatar has been successfully updated.',
        'avatar_from_gallery' => 'Avatar has been successfully set from the gallery.',
        'avatar_error' => 'The selected image does not have an attached media.',
        'player_card' => [
            'position_player' => 'Player',
            'my_teams' => 'My Teams',
            'status' => 'Status',
            'active_member' => 'Active Member',
        ],
        'section_settings' => [
            'title' => 'Member Section Settings',
            'default_team' => 'Default team to display',
            'default_team_help' => 'This team will be automatically selected for you after login on the dashboard and in the program.',
            'view_all' => 'All teams',
            'view_all_short' => 'All',
            'view_all_help' => 'If checked, you will see trainings and matches for all teams by default (can be switched in the header at any time).',
            'my_teams_only' => 'Default to my teams only',
            'active_view' => 'Active View',
        ],
        'help' => [
            'title' => 'Need help?',
            'text' => 'If you need to change sensitive data (such as club role or team assignment), please contact your coaches or club management.',
        ],
    ],
    'economy' => [
        'title' => 'Payments & Fees',
        'subtitle' => 'Overview of your membership fees and payments for ###TEAM_NAME### club.',
        'kpi' => [
            'total_to_pay' => 'Total to Pay',
            'overdue' => 'Overdue',
            'paid_total' => 'Total Paid',
        ],
        'to_pay_title' => 'Items to Pay',
        'no_open_charges' => 'You currently have no outstanding charges. Great job!',
        'charge_types' => [
            'membership_fee' => 'Fee',
            'camp_fee' => 'Camp',
            'tournament_fee' => 'Tournament',
            'other' => 'Other',
        ],
        'overdue_badge' => 'Overdue',
        'due_date' => 'Due by:',
        'paid_amount' => 'Paid:',
        'recent_payments' => 'Recent Payments',
        'no_recent_payments' => 'No payments have been recorded yet.',
        'no_variable_symbol' => 'No VS',
        'paid_charges' => 'Paid Items',
        'no_paid_charges' => 'You have no fully paid charges.',
        'bank_info' => [
            'title' => 'Bank Details',
            'text' => 'For regular membership fees, please use our transparent account:',
            'account_number' => 'Account Number',
            'bank' => 'Bank',
            'recipient_message' => 'Message for Recipient',
            'qr_payment' => 'QR Payment',
            'amount' => 'Amount (CZK)',
            'note' => 'Note (MSG)',
            'ss' => 'Specific Symbol (SS)',
            'note_placeholder' => 'Member name',
            'ss_placeholder' => 'Optional',
            'amount_placeholder' => '0.00',
            'any_amount' => 'Any amount',
            'download_qr' => 'Save to device',
        ],
    ],
    'notifications' => [
        // Translation keys for in-app notifications
        'title' => 'Notifications',
        'subtitle' => 'Overview of your alerts and messages.',
        'mark_all_read' => 'Mark all as read',
        'mark_read' => 'Mark as read',
        'new' => 'New',
        'default_title' => 'Alert',
        'default_action_label' => 'Show detail',
        'view_all' => 'View all notifications',
        'no_notifications' => 'No new notifications',
        'no_notifications_text' => 'You currently have no unresolved alerts.',
        'marked_read' => 'Notification has been marked as read.',
        'marked_all_read' => 'All notifications have been marked as read.',

        'rsvp_changed_title' => 'RSVP Change',
        'view_program' => 'View program',
        'rsvp_message_user' => ':label ":title":datetime: :status',
        'rsvp_message_self' => ':label ":title":datetime: :status',

        'rsvp_actions_self' => [
            'confirmed' => 'You confirmed attendance',
            'declined' => 'You submitted an excuse',
            'maybe' => 'You changed attendance to "maybe"',
            'changed' => 'You changed RSVP',
        ],
        'rsvp_actions_user' => [
            'confirmed' => ':name confirmed attendance',
            'declined' => ':name submitted an excuse',
            'maybe' => ':name changed attendance to "maybe"',
            'changed' => ':name changed RSVP',
        ],
        'rsvp_message' => ':action for :label :title:datetime',

        'event_labels' => [
            'training' => 'Training',
            'match' => 'Match',
            'club_event' => 'Club event',
            'event' => 'Event',
        ],

        'rsvp_statuses' => [
            'confirmed' => 'attending',
            'declined' => 'excused',
            'maybe' => 'maybe',
            'changed' => 'changed',
        ],

        'new_charge_title' => 'New Charge',
        'new_charge_message' => 'New charge ":title" issued for amount :amount CZK.',
        'view_payments' => 'View my payments',

        'mail' => [
            'subject_default' => 'Alert | :club',
            'greeting' => 'Hi :name!',
            'footer' => 'Thank you for being part of the team!',
            'salutation' => 'Your :club team',
            'view_portal' => 'View in portal',
            'new_charge_subject' => 'New Charge: :title',
            'charge_item' => 'Item: :title',
            'charge_amount' => 'Amount: :amount CZK',
            'charge_due_date' => 'Due date: :date',
            'charge_due_date_unknown' => 'not specified',
            'charge_please_pay' => 'Please pay on time. Thank you!',
        ],
    ],
    'teams' => [
        'title' => 'Team Overviews',
        'subtitle' => 'List of teams and their current attendance and roster status.',
        'detail_subtitle' => 'Team overview and upcoming events.',
        'back_to_list' => 'Back to team list',
        'no_category' => 'Category not specified',
        'stats' => [
            'players' => 'Players',
            'players_in_team' => 'Players in team',
            'trainings' => 'Trainings',
            'trainings_week' => 'Trainings (week)',
            'matches' => 'Matches',
            'upcoming_matches' => 'Upcoming matches',
            'avg_attendance' => 'Avg. attendance',
        ],
        'attendance' => [
            'matches_title' => 'Match Attendance',
            'trainings_title' => 'Training Attendance',
            'no_matches' => 'No upcoming matches.',
            'no_trainings' => 'No upcoming trainings.',
            'location_not_specified' => 'Location not specified',
            'match_badge' => 'Match',
            'confirmed' => 'Attending',
            'declined' => 'Excused',
            'confirmed_count' => ':count confirmed',
            'admin_detail' => 'Detail in admin',
        ],
        'view_detail' => 'View team detail',
        'no_teams' => 'No teams',
        'no_teams_text' => 'No teams found to display.',
    ],
    'search' => [
        'title' => 'Search Results',
        'query_label' => 'Search query:',
        'no_results' => 'No relevant destinations found',
        'no_results_text' => 'Try describing your query in different words or check for typos.',
        'back_to_dashboard' => 'Back to dashboard',
        'ai_recommendation' => 'AI Recommendation',
        'ai_recommendation_text' => 'Based on your query, we have selected the most likely places you might want to visit.',
        'not_found_title' => 'Did not find what you were looking for?',
        'not_found_text' => 'The system is constantly learning. If you feel the search should respond better to your query, let us know.',
        'send_feedback' => 'Send feedback',
    ],

    'loading' => [
        'generic' => [
            'Working on it...',
            'Dribbling to the goal...',
            'Passing the data...',
            'Falcon is flying for the answer...',
        ],
        'page' => 'Loading section :title...',
        'dashboard' => [
            'Inflating balls for your overview...',
            'Polishing the court on your dashboard...',
            'Dribbling to your stats...',
            'Setting up today\'s roster...',
        ],
        'attendance' => [
            'Blowing the whistle for training...',
            'Checking who didn\'t oversleep...',
            'Adding you to the scoresheet...',
            'Looking for your shoes in the locker room...',
        ],
        'profile' => [
            'Tailoring your jersey...',
            'Changing the number on your back...',
            'Taking your photo for the player card...',
            'Cleaning up your stats...',
        ],
        'economy' => [
            'Counting bribes for the refs... (just kidding!)',
            'Collecting for new basketballs...',
            'Searching for lost coins in the locker room...',
            'Printing club currency...',
        ],
        'teams' => [
            'Setting up the starting five...',
            'Drawing tactics on the board...',
            'Motivating the team in the locker room...',
            'Fine-tuning the form for the next game...',
        ],
        'notifications' => [
            'Sending out game invites...',
            'Passing secret signals...',
            'Sounding the alarm...',
            'Falcon is bringing a new message...',
        ],
        'search' => [
            'Looking for an open teammate...',
            'Scouting for new talents...',
            'Searching the whole arena...',
            'Going on a fast break for results...',
        ],
        'ai' => [
            'Falcon is taking a timeout to think...',
            'Coach AI is drawing a new play...',
            'Falcon is analyzing your crossover...',
            'AI Falcon is dunking into the bucket of knowledge...',
        ],
    ],

    'feedback' => [
        'contact_coach_title' => 'Contact Coach',
        'contact_coach_subtitle' => 'Send a message to your team coaches.',
        'contact_admin_title' => 'Contact Admin',
        'contact_admin_subtitle' => 'Report a system issue or request a setting change.',
        'team' => 'Team',
        'choose_team' => 'Choose team',
        'subject' => 'Subject',
        'message' => 'Message',
        'attachment' => 'Attachment',
        'attachment_help' => 'Optional. Max 10 MB. Allowed formats: pdf, jpg, png, doc, xls.',
        'send_to_coach' => 'Send to Coach',
        'send_to_admin' => 'Send to Admin',
        'no_team_warning' => 'You are not assigned to any team. The message will be delivered to the administrator.',
        'sent_success' => 'Your message has been sent successfully. Thank you for the feedback!',
        'hints' => [
            'economy' => 'For payments, your team coach is the best first contact. Please write to them using this form. Contact the administrator only if the coach does not respond.',
            'profile' => 'If you need to change sensitive data (such as club role or team assignment), please contact your coaches or club management.',
        ],
        'contact_card' => [
            'admin_title' => 'Administrator Contact',
            'coach_title' => 'Coach Contact',
            'coaches_title' => 'Team Coaches',
            'email' => 'Email',
            'phone' => 'Phone',
            'not_available' => 'Not available',
            'fallback_admin' => 'If no coach is listed, the message will be delivered to the administrator.',
            'choose_team_hint' => 'Choose a team to display coaches.',
            'admin_name_default' => 'Administrator',
        ],
    ],
    'roles' => [
        'super_admin' => 'Super Admin',
        'admin' => 'Administrator',
        'coach' => 'Coach',
        'editor' => 'Editor',
        'player' => 'Player',
        'parent' => 'Parent',
        'member' => 'Member',
        'treasurer' => 'Treasurer',
    ],
    'is_me' => '(Me)',
    'currency_czk' => 'CZK',
    'status_ok' => 'OK',
    'external_stats' => [
        'title' => 'History and Career (cz.basketball)',
        'season_competition' => 'Season / Competition',
        'team' => 'Team',
        'games_short' => 'G',
        'points_avg_short' => 'PTS Ø',
        'efficiency_avg_short' => 'EFF Ø',
        'rebounds_avg_short' => 'REB Ø',
        'free_throws_pct_short' => 'FT %',
        'career_total' => 'Career Total',
        'last_matches' => 'Last Matches (cz.basketball)',
        'date_competition' => 'Date / Competition',
        'opponent' => 'Opponent',
        'points' => 'Points',
        'efficiency' => 'EFF',
        'rebounds' => 'REB',
        'assists' => 'AST',
        'steals' => 'STL',
        'fouls' => 'F-',
        'sync_info' => 'These statistics are synchronized from the external portal :url. They may differ from our internal calculations depending on the quality and speed of data delivery from the CBF side.',
    ],
];
