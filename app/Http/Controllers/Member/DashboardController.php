...
class DashboardController extends Controller
{
    ...
    public function __invoke(Request $request): View
    {
        ...
        $isImpersonated = session()->has('impersonated_by');
        $cacheKey = "member_dashboard_{$user->id}_{$locale}_{$activeTeamId}_v2";

        // V režimu impersonace cache obcházíme pro bezpečnost a čerstvost dat
        $data = $isImpersonated
            ? $this->getDashboardData($user, $now, $activeTeamId)
            : Cache::remember($cacheKey, 300, fn () => $this->getDashboardData($user, $now, $activeTeamId));
     ...
     */
    protected function getDashboardData($user, $now, $activeTeamId): array
    {
