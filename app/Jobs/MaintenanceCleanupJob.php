...
class MaintenanceCleanupJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        $deletedAuditCount = AuditLog::where('occurred_at', '<', now()->subDays(90))->delete();

        // 3. Not Found logy (retence 30 dní)
        $deletedNotFoundCount = NotFoundLog::where('created_at', '<', now()->subDays(30))->delete();

        // 4. Externí importy (retence 30 dní)
        $deletedImportRuns = ExternalImportRun::where('created_at', '<', now()->subDays(30))->delete();
