            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <form action="{{ route('public.matches.index') }}" method="GET" class="flex flex-wrap items-center gap-4 py-4 md:py-0">
                    ...
                    @if($teamId || $matchType || ($seasonId && (string)$seasonId !== (string)($seasons->where('name', \App\Models\Season::getExpectedCurrentSeasonName())->first()?->id)))
                        <a href="{{ route('public.matches.index', ['type' => $type]) }}" class="flex items-center gap-2 px-3 py-2 bg-slate-100 hover:bg-danger/10 text-slate-500 hover:text-danger rounded-lg transition-all text-[10px] font-black uppercase tracking-widest border border-slate-200 shadow-sm" title="{{ __('matches.clear_filters') }}">
                            <i class="fa-light fa-circle-xmark text-sm"></i>
                            <span class="hidden sm:inline">{{ __('matches.clear_filters') }}</span>
                        </a>
                    @endif
                </form>
            </div>
