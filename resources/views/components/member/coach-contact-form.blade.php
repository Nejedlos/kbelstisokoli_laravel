...
new class extends Component {
    ...
    public function mount()
    {
        ...
        if ($teams->count() === 1) {
            $this->teamId = (string)$teams->first()->id;
        }
    }

    public function getTeamsProperty()
    {
        return auth()->user()->playerProfile?->teams()->with('coaches')->get() ?? collect();
    }

    public function getCoachContactsProperty()
    {
        if (!$this->teamId) return [];

        $team = Team::with('coaches')->find($this->teamId);
        if (!$team) return [];
    ...
    public function getAdminFallbackProperty()
    {
        ...
        <div class="card sport-card-accent p-6 md:p-10">
            <form wire:submit.prevent="send" class="space-y-8">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-6">
                        @php $teamsCount = $this->teams->count(); @endphp
                        ...
                        @else
                            <div class="space-y-2">
                                ...
                                <div class="relative">
                                    <select id="team_id" wire:model.live="teamId" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-club focus:ring-2 focus:ring-primary focus:border-primary transition-all font-bold text-secondary appearance-none">
                    ...
                    <div class="space-y-6">
                        <div class="space-y-3">
                            @php $contacts = $this->coachContacts; @endphp
                            ...
                            @elseif($teamId || $teamsCount === 1)
                                {{-- Fallback na admina pro vybraný tým bez trenérů --}}
                                @php $admin = $this->adminFallback; @endphp
