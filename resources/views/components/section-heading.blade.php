...
<div class="mb-10 {{ $align === 'center' ? 'text-center' : 'text-left' }}">
    @if($subtitle)
        <div class="text-[min(3.5vw,0.875rem)] sm:text-sm uppercase tracking-widest-responsive text-primary font-bold mb-3 badge-nowrap max-w-full overflow-hidden leading-tight">{{ $subtitle }}</div>
    @endif
    @if($title)
