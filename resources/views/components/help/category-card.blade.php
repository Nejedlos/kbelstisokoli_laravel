...
<a href="{{ \App\Filament\Pages\Help::getUrl(['cat' => $category->slug]) }}"
   class="group relative bg-white rounded-[2rem] border border-slate-100 p-10 hover:shadow-xl hover:border-primary-200 hover:-translate-y-1 transition-all duration-500 overflow-hidden flex flex-col h-full">
    ...
    <div @class([
        ...
        'bg-blue-500' => ($category->color ?? 'slate') === 'blue',
        'bg-green-500' => ($category->color ?? 'slate') === 'green',
    ...
    <div class="flex items-center justify-between mt-auto pt-4">
        ...
        <div class="w-12 h-12 rounded-full flex items-center justify-center transition-all duration-500 bg-slate-50 text-slate-300 group-hover:bg-primary-600 group-hover:text-white group-hover:shadow-lg group-hover:shadow-primary-600/30">
