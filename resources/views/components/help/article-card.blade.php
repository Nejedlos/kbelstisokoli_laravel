   ...
   @class([
    ...
    @if($isMatchingRole)
        <div class="absolute top-0 right-0">
            <div class="bg-primary-500 text-white text-[10px] font-black uppercase tracking-tighter px-3 py-1 rounded-bl-xl shadow-sm animate-pulse">
                {{ __('admin.help.for_you') }}
            </div>
        </div>
    @elseif($isFeatured)
        <div class="absolute top-0 right-0">
            <div class="bg-amber-500 text-white text-[10px] font-black uppercase tracking-tighter px-3 py-1 rounded-bl-xl shadow-sm">
    ...
    <div class="flex items-center gap-6">
        <div @class([
            ...
            <i @class([
                ...
                'text-slate-500 group-hover:text-primary-600' => !$isMatchingRole,
        ...
        <div>
            ...
            @if(isset($article->search_excerpt))
                <div class="text-sm text-slate-600 font-medium leading-relaxed mt-1 search-excerpt">
                    {!! $article->search_excerpt !!}
                </div>
            @elseif($excerptStr)
                <p class="text-sm text-slate-600 line-clamp-1 font-medium">{{ $excerptStr }}</p>
            @endif
        </div>
    </div>
    <div class="flex items-center gap-4 shrink-0">
        ...
        @endif
        <i class="fa-light fa-chevron-right text-slate-400 group-hover:text-primary-600 group-hover:translate-x-1 transition-all"></i>
    </div>
</a>
