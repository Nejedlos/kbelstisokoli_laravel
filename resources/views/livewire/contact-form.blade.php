...
>
    @if($success)
        <div class="bg-emerald-50 border border-emerald-100 p-8 rounded-[2rem] text-center animate-fade-in" role="alert">
            ...
            <h3 class="text-2xl font-black text-emerald-900 uppercase tracking-tight mb-2">{{ __('contact.success_title') }}</h3>
        ...
        >
            ...
            @if($toEmail)
                <div class="bg-slate-50 p-4 rounded-2xl border border-slate-100 flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-[10px] font-black uppercase tracking-widest text-slate-400 leading-none mb-1">{{ __('contact.recipient') }}</p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                    <label for="name" class="block text-xs font-black uppercase tracking-widest text-slate-500 ml-1">{{ __('contact.your_name') }}</label>
                    <div class="relative group">
                        ...
                        <input wire:model="name" type="text" id="name" class="w-full bg-slate-50 border-slate-200 rounded-2xl pl-11 py-3 text-sm focus:border-primary focus:ring-primary/20 transition-all" placeholder="{{ __('contact.name_placeholder') }}" required>
                    </div>
                    @error('name') <span class="text-xs text-rose-600 font-bold tracking-tight ml-1">{{ $message }}</span> @enderror
                </div>
                <div class="space-y-2">
                    <label for="email" class="block text-xs font-black uppercase tracking-widest text-slate-500 ml-1">{{ __('contact.your_email') }}</label>
                    <div class="relative group">
                        ...
                        <input wire:model="email" type="email" id="email" class="w-full bg-slate-50 border-slate-200 rounded-2xl pl-11 py-3 text-sm focus:border-primary focus:ring-primary/20 transition-all" placeholder="{{ __('contact.email_placeholder') }}" required>
                    </div>
            ...
            <div class="space-y-2">
                <label for="subject" class="block text-xs font-black uppercase tracking-widest text-slate-500 ml-1">{{ __('contact.subject_label') }}</label>
                <div class="relative group">
                    ...
                    <input wire:model="subject" type="text" id="subject" class="w-full bg-slate-50 border-slate-200 rounded-2xl pl-11 py-3 text-sm focus:border-primary focus:ring-primary/20 transition-all" placeholder="{{ __('contact.subject_placeholder') }}" required>
                </div>
                @error('subject') <span class="text-xs text-rose-600 font-bold tracking-tight ml-1">{{ $message }}</span> @enderror
            </div>

            <div class="space-y-2">
                <label for="message" class="block text-xs font-black uppercase tracking-widest text-slate-500 ml-1">{{ __('contact.message_label') }}</label>
                <textarea wire:model="message" id="message" rows="5" class="w-full bg-slate-50 border-slate-200 rounded-2xl p-4 text-sm focus:border-primary focus:ring-primary/20 transition-all" placeholder="{{ __('contact.message_placeholder') }}" required></textarea>
                @error('message') <span class="text-xs text-rose-600 font-bold tracking-tight ml-1">{{ $message }}</span> @enderror
            </div>

            <div class="space-y-2">
                <label for="attachment" class="block text-xs font-black uppercase tracking-widest text-slate-500 ml-1">{{ __('contact.attachment_label') }}</label>
                <div class="flex items-center justify-center w-full">
                    <label for="attachment" class="flex flex-col items-center justify-center w-full h-32 border-2 border-slate-200 border-dashed rounded-2xl cursor-pointer bg-slate-50 hover:bg-slate-100 transition-all">
                        <div class="flex flex-col items-center justify-center pt-5 pb-6">
                            ...
                            <p class="mb-1 text-sm text-slate-500 font-bold">{{ __('contact.attachment_select') }}</p>
                            <p class="text-xs text-slate-400 uppercase tracking-widest">{{ __('contact.attachment_formats') }}</p>
                        </div>
            ...
            @if($recaptchaEnabled && $siteKey)
                <div class="flex items-center gap-4 bg-emerald-50/50 p-5 rounded-[1.5rem] border border-emerald-100/50 group/captcha transition-all hover:bg-emerald-50 hover:border-emerald-200">
                    ...
                    <div class="flex-1">
                        <p class="text-[10px] font-black uppercase tracking-widest text-emerald-800 mb-0.5 leading-none">{{ __('contact.antispam_active') }}</p>
                        <p class="text-[10px] text-emerald-600/80 leading-tight">
                            {!! __('contact.antispam_desc') !!}
                        </p>
                    </div>
